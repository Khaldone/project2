...
public class ModerationDemo : MonoBehaviour {
    ...
    private void Start() {
        ...
        if (testImage != null) {
            var sendImageButton = new Button(() => SendImageRequest()) {
                text = "Test Image Moderation"
            };
            sendImageButton.tooltip = "Send the test image for moderation.";
            uiDocument.rootVisualElement.Add(sendImageButton);
        }
    }
    ...
    private void SelectMessage(VisualElement messageElement) {
        if (!_messageResponses.ContainsKey(messageElement)) return;
    ...
    private void SendImageRequest() {
        ...
        var messageLabel = messageElement.Q<Label>("message-text");
        messageLabel.text = "[Image Moderation Test]";
        ...
        _cancelCallback = Moderation.RequestImage(testImage, parameters, completeCallback: response => {
            var flagged = IsPG13Appropriate(response);
            var highestScore = response.GetHighestScore();
            ...
            string t = string.Empty;
            t += flagged ? "<b><color=red>❌ NOT PG13 - IMAGE FLAGGED</color></b>\n\n" : "<b><color=green>✓ PG13 APPROPRIATE</color></b>\n\n";

            t += "<b>Category Breakdown:</b>\n";
            var categoryScores = response.results[0].category_scores;
            CheckAndAddCategory(ref t, "sexual", categoryScores.sexual, 0.09f, flaggedColor, safeColor);
            CheckAndAddCategory(ref t, "sexual/minors", categoryScores.sexualMinors, 0.0f, flaggedColor, safeColor);
            CheckAndAddCategory(ref t, "violence", categoryScores.violence, 0.25f, flaggedColor, safeColor);
            CheckAndAddCategory(ref t, "violence/graphic", categoryScores.violenceGraphic, 0.1f, flaggedColor, safeColor);
            CheckAndAddCategory(ref t, "harassment", categoryScores.harassment, 0.25f, flaggedColor, safeColor);
            CheckAndAddCategory(ref t, "hate", categoryScores.hate, 0.25f, flaggedColor, safeColor);
            CheckAndAddCategory(ref t, "hate/threatening", categoryScores.hateThreatening, 0.15f, flaggedColor, safeColor);
            CheckAndAddCategory(ref t, "self-harm", categoryScores.selfHarm, 0.02f, flaggedColor, safeColor);
    ...
    private void CheckAndAddCategory(ref string text, string categoryName, float score, float threshold, Color flaggedColor, Color safeColor) {
        var isFlagged = score > threshold;
        var categoryColor = isFlagged ? flaggedColor : safeColor;
        var categoryColorString = $"#{ColorUtility.ToHtmlStringRGB(categoryColor)}";
        var flagIcon = isFlagged ? "⚠️" : "✓";
        text += $"{flagIcon} <color={categoryColorString}>{categoryName}: {score:0.000} (threshold: {threshold:0.000})</color>\n";
    }

    private bool IsPG13Appropriate(ModerationResponse response) {
        if (response.results.Length == 0) return true;

        var result = response.results[0];
        if (result.flagged) return true;

        if (result.category_scores.sexual > 0.09f) return true;
        if (result.category_scores.sexualMinors > 0.0f) return true;
        if (result.category_scores.violence > 0.25f) return true;
        if (result.category_scores.violenceGraphic > 0.1f) return true;
        if (result.category_scores.harassment > 0.25f) return true;
        if (result.category_scores.hate > 0.25f) return true;
        if (result.category_scores.hateThreatening > 0.15f) return true;
        if (result.category_scores.selfHarm > 0.02f) return true;
