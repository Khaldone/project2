...
public class ModerationParametersDrawer : PropertyDrawer {
    ...
    public override float GetPropertyHeight(SerializedProperty property, GUIContent label) {
        // Compute the height directly from the SerializedProperty state instead of
        // depending on _height being set by a prior OnGUI call. The previous "return
        // _height" pattern would return 0 on the very first layout pass (before OnGUI
        // had ever run), causing the drawer to overflow its rect and break the layout
        // of every field declared after this one on the same MonoBehaviour.
        ...
        // Initial gap inside the expanded block.
        h += space + space;

        // API Key row + optional "API Key is required" helpbox & button.
        var apiKeyProp = property.FindPropertyRelative(nameof(ModerationParameters.apiKey));
        var apiKeyEmpty = string.IsNullOrEmpty(apiKeyProp.stringValue);
        if (apiKeyEmpty) {
            // Two-line helpbox + a button row.
            h += line * 2 + space;
            h += line + space;
        }
        h += line + space;
        ...
        // Encryption row.
        var encryptionProp = property.FindPropertyRelative(nameof(ModerationParameters.apiKeyEncryption));
        var encryption = (ApiKeyEncryption)encryptionProp.enumValueIndex;

        // Optional "API Key is not encrypted" warning helpbox (when None + key is set).
        if (encryption == ApiKeyEncryption.None && !apiKeyEmpty) {
            h += line * 2 + space;
        }
        h += line + space; // encryption enum popup itself

        // Conditional fields based on encryption mode.
        if (encryption == ApiKeyEncryption.LocallyEncrypted) {
            var passwordProp = property.FindPropertyRelative(nameof(ModerationParameters.apiKeyEncryptionPassword));
            if (string.IsNullOrEmpty(passwordProp.stringValue)) {
                // "Password is required" helpbox.
                h += line + space;
            }
            h += line + space; // password text field
        }
        else if (encryption == ApiKeyEncryption.RemoteConfig) {
            h += line + space; // remote config key field
        }

        // Inter-section space.
        h += space + space;

        // Model, Timeout, Throttle rows.
        h += (line + space) * 3;

        // Trailing space + group spacing applied by the drawer.
        h += space * 3;
