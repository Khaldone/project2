...
namespace ibc.solvers
{
    ...
    {
        public bool IsCreated => Balls.IsCreated;
        ...
        public PhysicsScene(PhysicsScene scene, Allocator allocator)
        {
            Balls = new NativeArray<Ball>(scene.Balls, allocator);
            Holes = new NativeArray<Hole>(scene.Holes, allocator);
            Cushions = new NativeArray<Cushion>(scene.Cushions, allocator);
    ...
    public partial struct PhysicsSolver
    {
        ...
        public struct Event
        {
            ...
            public float3 Ball1Velocity;
            public float3 Ball2Velocity;

            public string ToString(BilliardManagedScene scene)
            {
                switch (Type)
                {
                    case EventType.None:
                    ...
                    case EventType.StateTransition:
                        return $"{Type} in {Time} to {Motion} for id: {scene.Balls[BallIndex].Identifier} ";
                    case EventType.BallCollision:
                        return $"{Type} in {Time} between id: {scene.Balls[BallIndex].Identifier} and id: {scene.Balls[OtherIndex].Identifier} ";
                    case EventType.CushionCollision:
                        return $"{Type} in {Time} between id: {scene.Balls[BallIndex].Identifier} and cushion id: {scene.Cushions[OtherIndex].Identifier}";
                    case EventType.PocketCollision:
                        return $"{Type} in {Time} id: {scene.Balls[BallIndex].Identifier}";
                    default:
        ...
        private RootResult RunPolySolver(double3 A, double3 B, double3 C, double D, double timeBound)
        {
            double root;
            ...
            var roots = poly.CalculateRoots(timeBound + _constants.PolynomialErrorBound, _constants.PolynomialErrorBound);
            return roots.TrimRoots(0, timeBound + _constants.PolynomialErrorBound).Sort();
        }

        /// <summary> Returns time of impact between two balls. </summary>
        private RootResult GetBall2BallCollisionEvent(Ball ball1, Ball ball2, double timeBound)
        {
        ...
        private RootResult GetBall2PointCollisionEvent(Ball ball, double3 p0, double timeBound)
        {
            if (math.lengthsq(ball.Position - p0) < ball.Radius * ball.Radius)
                return new RootResult(0);
            

            var roots = RunPolySolver(ball.AccCoeff, ball.VelCoeff, ball.Position - p0, ball.Radius, timeBound);
            var validRoots = new RootResult();
            for (int i = 0; i < roots.Count; ++i)
            {
                ...
                double newDist = math.length(p0 - newPos);
        ...
        private (bool, double, int) GetBall2CushionCollisionEvent(Ball ball, Cushion cushion, double timeBound)
        {
            ...
            if (math.lengthsq(cp - ball.Position) < ball.Radius * ball.Radius)
                return (true, 0, -1);
            ...
            var rootResult = RunPolySolver(A, B, C, ball.Radius, timeBound);
            var bestRoot = Infinity;
            var vertex = -1;
            var rootFound = false;
            
            for (int i = 0; i < rootResult.Count; ++i)
            {
                (bool success, double root, int vertex) result = Ball2CushionCollisionEvent(ball, p0, p1, cushion.Dir, cushion.Distance, timeBound, rootResult[i]);
                if (result.success && result.root < bestRoot)
                {
                    bestRoot = result.root;
                    vertex = result.vertex;
                    rootFound = true;
                }
            }
            
            var newBound = rootFound ? bestRoot : timeBound;
            
            {
                //check if sphere intersects line segment end points
                var r1 = GetBall2PointCollisionEvent(ball, p0, newBound);
                if (r1.Count > 0)
                {
                    if (r1.Root1 < bestRoot)
                    {
                        newBound = r1.Root1;
                        bestRoot = r1.Root1;
                        vertex = 0;
                ...
                var r2 = GetBall2PointCollisionEvent(ball, p1, newBound);
                if (r2.Count > 0)
                {
                    if (r2.Root1 < bestRoot)
                    {
                        newBound = r2.Root1;
                        bestRoot = r2.Root1;
                        vertex = 1;
            ...
            return (rootFound, bestRoot, vertex);
        }

        private (bool, double, int) Ball2CushionCollisionEvent(Ball ball, double3 p0, double3 p1, double3 dir, double distance, double timeBound, double root)
        {
            //check if point lies on the segment
            double3 newPos = (ball.AccCoeff * root + ball.VelCoeff) * root + ball.Position;
            double3 newCp = GetClosestPointOnLineSegment(newPos, p0, dir, distance);
            ...
            {
                //check if sphere intersects line segment end points
                var r1 = GetBall2PointCollisionEvent(ball, p0, timeBound);
                if (r1.Count > 0)
                {
                    return (true, r1.Root1, 0);
                }


                var r2 = GetBall2PointCollisionEvent(ball, p1, timeBound);
                if (r2.Count > 0)
                {
                    return (true, r2.Root1, 1);
                }
            }
        ...
        private RootResult GetBall2PocketCollisionEvent(Ball ball, Hole hole, double timeBound)
        {
            var dist = ball.Radius + hole.Radius;
            var distSq = dist * dist;
            var relativePos = ball.Position - hole.Position;
            if (math.lengthsq(relativePos) < distSq)
                return new RootResult(0);

            return RunPolySolver(ball.AccCoeff, ball.VelCoeff, relativePos, dist, timeBound);
        }
        ...
        public Event GetNextEvent(PhysicsScene scene)
        {
            ...
            for (int i = 0; i < balls.Length; ++i)
            {
                ...
                for (int j = 0; j < cushions.Length; ++j)
                {
                    ...
                    (bool success, double root, int vertex) = GetBall2CushionCollisionEvent(balls[i], cushions[j], e.Time);
                    if(!success) continue;
                    if (e.Time <= root) continue;
            ...
            for (int i = 0; i < balls.Length; ++i)
            {
                ...
                for (int j = i + 1; j < balls.Length; ++j)
                {
                    ...
                    var roots = GetBall2BallCollisionEvent(balls[i], balls[j], e.Time);
                    if(roots.Count == 0) continue;
                    if (e.Time <= roots[0]) continue;
            ...
            for (int i = 0; i < balls.Length; ++i)
            {
                ...
                for (int j = 0; j < holes.Length; ++j)
                {
                    ...
                    var roots = GetBall2PocketCollisionEvent(balls[i], holes[j], e.Time);
                    if(roots.Count == 0) continue;
                    if (e.Time <= roots[0]) continue;
            ...
            {
                StepBallState(balls[e.BallIndex], out var temp, e.Time);
                e.Ball1Velocity = (float3) temp.Velocity;
                
                if (e.Type == EventType.BallCollision)
                {
                    temp = balls[e.OtherIndex];
                    StepBallState(balls[e.OtherIndex], out temp, e.Time);
                    e.Ball2Velocity = (float3) temp.Velocity;
                }
            }
        ...
        public void Step(PhysicsScene scene, NativeArray<Ball> balls, double dt)
        {
            for (var i = 0; i < balls.Length; i++)
            {
                var ball = balls[i];
                quaternion origRot = ball.Rotation;
                StepBallState(scene.Balls[i], out ball, dt);
                ball.Rotation = origRot;
                balls[i] = ball;
            }
        }
        ...
        private double AirborneTime(Ball b)
        {
            ...
            var roots = poly2.CalculateRoots(Infinity);
            if (roots.Count > 0)
        ...
        public bool ResolveBallCueImpact(
            ref Ball ball, 
            in Cue cue, 
            double velocity, 
            double jaw, 
            ...
            double2 offset)
        {
            PhysicsImpactParameters impactParams = cue.ImpactParameters;
            
            double ballRadius = ball.Radius;
            double cueHalfLength = cue.Length / 2.0;
            double friction = impactParams.Friction;

            // Contact point in cue stick frame (z-axis aligned with stick)
            double3 contactPoint = new double3(offset.x, offset.y, 0) * ballRadius;

            // More forgiving miscue check similar
            double maxAllowedOffset = ballRadius * 0.9;
            double contactDistance = math.length(contactPoint.xy);
            
            if (contactDistance > maxAllowedOffset)
            {
                // Scale down the offset to maximum allowed 
                double scale = maxAllowedOffset / contactDistance;
                contactPoint.xy *= scale;
            }

            // Update z component for contact point
            double contactXYSq = math.lengthsq(contactPoint.xy);
            double radiusSq = ballRadius * ballRadius;
            
            // Ensure we don't get NaN from sqrt
            if (contactXYSq >= radiusSq)
            {
                contactXYSq = radiusSq * 0.99;
            }
            
            contactPoint.z = -math.sqrt(radiusSq - contactXYSq);

            // Inverse angular inertia matrices
            double3x3 invInertiaBall = double3x3.identity * ball.InverseInertia;
            double3x3 invInertiaCue = double3x3.identity * cue.InverseInertia;

            // Stick frame from world frame
            double3x3 stickRotation = EulerZXY(new double3(math.radians(pitch), math.radians(jaw), 0));

            // Contact point in world frame
            double3 contactWorldPos = math.mul(stickRotation, contactPoint);

            // Calculate positions
            double3 ballCenter = double3.zero;
            double3 cueCenter = contactWorldPos + math.mul(stickRotation, new double3(0, 0, -cueHalfLength));

            // Calculate inverse inertia matrix
            double3x3 invInertiaMatrix = CalculateInverseInertiaMatrix(
                ballCenter, cueCenter, contactWorldPos,
                double3x3.identity, stickRotation,
                ball.InverseMass, cue.InverseMass,
                invInertiaBall, invInertiaCue);

            double3x3 inertiaMatrix = math.inverse(invInertiaMatrix);

            // Cue velocity and normal
            double3 cueVelocity = stickRotation.c2 * velocity;
            double3 normal = -stickRotation.c2;
            
            double effectiveFriction = friction;
            double offsetRatio = contactDistance / ballRadius;
            
            // Increase friction for off-center hits
            if (offsetRatio > 0.3)
            {
                effectiveFriction *= (1.0 + offsetRatio * 0.5);
            }

            double3 impulse = SimpleAlgebraicImpulse(
                inertiaMatrix, 
                cueVelocity, 
                normal, 
                effectiveFriction,
                impactParams.NormalRestitution, 
                impactParams.TangentialRestitution);

            // Apply velocities
            double3 newLinearVel = ball.Velocity - ball.InverseMass * impulse;
            double3 newAngularVel = ball.AngularVelocity - ball.InverseInertia * math.cross(contactWorldPos, impulse);

            // Add extra spin for off-center hits (power spin effect)
            if (offsetRatio > 0.2)
            {
                double spinBoost = 1.0 + (offsetRatio - 0.2) * 0.8;
                newAngularVel *= spinBoost;
            }

            // Clamp velocities to reasonable values to prevent physics explosions
            double maxLinearSpeed = velocity * 1.2; // Allow slight boost from spin
            double linearSpeed = math.length(newLinearVel);
            if (linearSpeed > maxLinearSpeed)
            {
                newLinearVel = math.normalize(newLinearVel) * maxLinearSpeed;
            }

            double maxAngularSpeed = 100.0; // Reasonable max spin
            double angularSpeed = math.length(newAngularVel);
            if (angularSpeed > maxAngularSpeed)
            {
                newAngularVel = math.normalize(newAngularVel) * maxAngularSpeed;
            }

            // Set final velocities
            SetVelocity(ref ball, newLinearVel);
            SetAngularVelocity(ref ball, newAngularVel);
        ...
        public static bool IsCollisionEvent(EventType ev)
        {
            return ev == EventType.BallCollision || ev == EventType.CushionCollision || ev == EventType.PocketCollision;
        }
    }
}