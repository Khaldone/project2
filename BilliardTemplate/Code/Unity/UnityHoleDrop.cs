...
namespace ibc
{
    ...
    public class UnityHoleDrop : MonoBehaviour
    {
        ...
        {
            var index = _objects.FindIndex(t=>t.Identifier == identifier);
            if (index == -1)
            {
                return false;
            }
        ...
        public void Reset(Ball[] pointBalls)
        {
            foreach (var pointBall in pointBalls)
            {
                if (pointBall.State != Ball.StateType.Pocketed)
                {
                    var index = _objects.FindIndex(t => t.Identifier == pointBall.Identifier);
                    if (index >= 0)
                        _objects.RemoveAt(index);
                }
            }
